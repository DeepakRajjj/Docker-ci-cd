...
const getData = async (prompt) =>{
    let data = await fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${prompt}`)
    ...
    div.innerHTML=`
        ...
        <p>Example: <span>${jsonData[0].meanings[0].definitions[0].example == "" ? "Not Found" : jsonData[0].meanings[0].definitions[0].example}</span></p>
        <p>Synonyms: <span>${jsonData[0].meanings[0].synonyms == "" ? "Not Found" : jsonData[0].meanings[0].synonyms}</span></p>
